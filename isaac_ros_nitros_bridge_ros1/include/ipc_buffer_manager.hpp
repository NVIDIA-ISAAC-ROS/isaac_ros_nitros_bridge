...
public:
  ...
  // Constructor, Initialize CUDA Buffers
  IPCBufferManager(size_t size, size_t buffer_step)
  {
    ...
    for (size_t i = 0; i < buffer_size_; i++) {
      ...
      if (cudaSuccess != cuda_err) {
        ROS_ERROR("Failed to malloc CUDA memory: %s", cudaGetErrorString(cuda_err));
      }
      ipc_mem_handle = new cudaIpcMemHandle_t;
      cuda_err = cudaIpcGetMemHandle(ipc_mem_handle, buffer_ptr);
      if (cudaSuccess != cuda_err) {
        ROS_ERROR("Failed to get CUDA IPC handle: %s", cudaGetErrorString(cuda_err));
      }
